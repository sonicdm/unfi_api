...
from settings import search_chunk_size, auto_download, default_save_path, auto_login, auto_save, username, password, save_settings
...
class SearchPage(TkFrame):
    ...
    def __init__(
        ...
        self.progress_bar_frame.grid(row=2, column=0, columnspan=2, sticky="nsew")
        # progress bars
        ...
        # options frame, has save path entry box and download button
        ...
        self.controller.store_tk_variable("results_listbox_variable", self.results_listbox_variable)
        self.controller.store_tk_variable("list_box_selected", self.list_box_selected)

        
        self.listbox_map: dict = {}
    ...
    def create_listbox_column_widgets(self):
        ...
        list_box_container.columnconfigure(0, weight=1)
        list_box_container.rowconfigure(0, weight=1)
        list_box_label = tk.Label(list_box_container, textvariable=self.results_listbox_variable, font=("Arial", 15))
        ...
        list_box.bind("<<ListboxSelect>>", lambda x: self.list_box_select(listbox=list_box))
        ...
        select_all_button = tk.Button(select_buttons_frame, text="Select All", command=lambda: self.list_box_select_all(list_box))
        select_none_button = tk.Button(select_buttons_frame, text="Select None", command=lambda: self.list_box_deselect_all(list_box))
    ...
    def create_search_entry_frame(self):
        ...
        search_entry_container.columnconfigure(0, weight=1)
        search_entry_container.rowconfigure(0, weight=1)
        search_label = tk.Label(search_entry_container, text="Search:", font=("Arial", 15))
        search_label.grid(row=0, column=0, columnspan=2, sticky="nsew")
        ...
        # search_entry_scroll_bar = tk.Scrollbar(
        #     search_entry_container, orient=tk.VERTICAL, command=self.search_entry.yview
        # )
        # self.search_entry.config(yscrollcommand = search_entry_scroll_bar.set)
        # search_entry_scroll_bar.grid(row=1, column=1, sticky="ns")
        ...
        # enable search button when search entry box has text
        self.search_entry.bind("<KeyRelease>", lambda x: self.search_entry_key_release())
    ...
    def do_download(self):
        ...
        messagebox.showinfo("Download", "Downloading:\n" + "\n".join([str(idx) + ": " + self.results_listbox.get(idx) for idx in selection]))
